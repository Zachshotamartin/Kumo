...
/** The editor's commands are independent of where the document is stored. */
export interface EditorRuntime {
  mutateShapes: (next: Shape[], previous: Shape[]) => void;
  mutateBackground: (color: string) => void;
  updateBoardSettings: (...args: Parameters<typeof updateBoardSettings>) => Promise<unknown>;
  cloneBoardAssets: typeof cloneBoardAssets;
...
export const useEditorActionsCore = ({
  ...
  const commitBoardPatch = useCallback(
    ...
    [board, canEdit, dispatch, mutateBackground, updateBoardSettings]
  );
  ...
  }, [board.id, board.shapes, canEdit, cloneBoardAssets, commitShapes, dispatch, editor.clipboard, editor.clipboardBoardId, editor.clipboardParentBounds]);
...
export type EditorActions = ReturnType<typeof useEditorActionsCore>;
