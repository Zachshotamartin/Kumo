...
import { cancelAccountDeletion, exportAccountData, loadNotificationPreferences, loadOperations, requestAccountDeletion, revokeAccountSessions, updateNotificationPreferences, type NotificationPreferences, type OperationsOverview } from "../../services/platformRepository";
...
export const SettingsView = () => {
  ...
  useEffect(() => { void Promise.all([loadNotificationPreferences(), loadOperations()]).then(([nextPreferences, nextOperations]) => { setPreferences(nextPreferences); setOperations(nextOperations); }).catch((caught) => setError(caught.message)); }, []);
  const update = async (patch: Partial<NotificationPreferences>) => {
    if (!preferences) return;
    if (patch.browser_enabled === true && !(await requestBrowserNotificationPermission())) {
      setError("Allow notifications in your browser before enabling Kumo browser alerts.");
      return;
    }
    const previous = preferences;
    const next = { ...preferences, ...patch };
    setPreferences(next);
    try { setPreferences(await updateNotificationPreferences(next)); setError(null); }
