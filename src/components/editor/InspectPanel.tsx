...
import { inspectTokens, shapeCss, shapeJson, shapeReact, shapeSwiftUI } from "../../editor/handoff";
...
const InspectPanel = () => {
  ...
  const [codeFormat, setCodeFormat] = useState<"css" | "react" | "swift" | "json">("css");
  ...
  const code = shape ? codeFormat === "css" ? shapeCss(shape) : codeFormat === "react" ? shapeReact(shape) : codeFormat === "swift" ? shapeSwiftUI(shape) : shapeJson(shape) : "";
  return (
      ...
      <div className={styles.inspectorBody}>
        {!shape || !bounds || !tokens ? <div className={styles.emptyPanel}><Code aria-hidden="true" /><p>Select a layer to inspect its implementation.</p></div> : (
          <>
            ...
            <section className={styles.inspectorSection}><h2>Handoff</h2><p className={styles.typeToken}>Status: {shape.devStatus ?? "designing"}</p>{shape.devAnnotation && <p>{shape.devAnnotation}</p>}{shape.codeComponentUrl && <a href={shape.codeComponentUrl} target="_blank" rel="noreferrer">Open code component</a>}{measurements.map((measurement) => <p className={styles.typeToken} key={measurement.axis}>{measurement.axis} gap: {Math.round(measurement.value)}px</p>)}<label className={styles.fullField}><span>Code format</span><select value={codeFormat} onChange={(event) => setCodeFormat(event.target.value as typeof codeFormat)}><option value="css">CSS</option><option value="react">React</option><option value="swift">SwiftUI</option><option value="json">JSON</option></select></label></section>
