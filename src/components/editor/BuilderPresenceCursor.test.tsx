...
it('shows actual world positions and expires disconnected builder cursors', () => {
  ...
  expect(screen.getByText('AI · Build').parentElement).toHaveStyle({ left: '40px', top: '40px' });
  act(() => vi.advanceTimersByTime(1000)); expect(screen.queryByText('AI · Build')).not.toBeInTheDocument(); rendered.unmount(); vi.useRealTimers();
});
