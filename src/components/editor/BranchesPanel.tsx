...
const BranchesPanel = () => {
  ...
  const load = useCallback(async () => {
    ...
    catch (caught) { setError(errorMessage(caught, "Branches could not be loaded.")); }
  ...
  const openReview = async (branchId: string) => {
    ...
    catch (caught) { setError(errorMessage(caught, "Diff failed.")); }
  };
  ...
  const updateFromMain = async (branchId: string, selectedResolutions = resolutions) => {
    ...
    try {
      ...
      setError(errorMessage(caught, "Branch update failed."));
    } finally { setLoading(false); }
  };
    ...
    <div className={styles.inspectorBody}>
      ...
      <section className={styles.inspectorSection}>
        ...
        <button type="button" disabled={!name.trim() || !board.id || board.role === "viewer" || loading} onClick={() => {
          setLoading(true); void createDesignBranch(board.id!, name).then((branch) => { setBranches((current) => [branch, ...current]); enter(branch); }).catch((caught) => setError(errorMessage(caught, "Branch creation failed."))).finally(() => setLoading(false));
      ...
      <section className={styles.inspectorSection}>
        ...
        <div className={styles.branchList}>{openBranches.map((branch) => <div className={styles.branchRow} key={branch.id}>
          ...
          <div>
            ...
            <button type="button" aria-label={`Copy link to ${branch.name}`} onClick={() => board.id && void navigator.clipboard.writeText(branchUrl(board.id, branch.id)).then(() => setMessage("Branch link copied.")).catch((caught) => setError(errorMessage(caught, "Branch link could not be copied.")))}><Copy aria-hidden="true" /></button>
            ...
            {confirmMerge === branch.id ? <button type="button" className={styles.mergeConfirm} onClick={() => {
              if (!board.id) return;
              setLoading(true); void mergeDesignBranch(board.id, branch.id, mergeDescription).then((result) => { leave(result.revision); setMergeDescription(""); return load(); }).catch((caught) => setError(errorMessage(caught, "Merge failed."))).finally(() => { setConfirmMerge(null); setLoading(false); });
            ...
            <button type="button" aria-label={`Archive ${branch.name}`} onClick={() => board.id && void archiveDesignBranch(board.id, branch.id).then(load).catch((caught) => setError(errorMessage(caught, "Archive failed.")))}><Archive aria-hidden="true" /></button>
            <button type="button" onClick={() => void openReview(branch.id)}>Review</button>
          </div>
          {renaming === branch.id && <form className={styles.branchReview} onSubmit={(event) => { event.preventDefault(); if (!board.id) return; void renameDesignBranch(board.id, branch.id, renameValue).then(() => { setRenaming(null); return load(); }).catch((caught) => setError(errorMessage(caught, "Rename failed."))); }}><label className={styles.fullField}><span>Branch name</span><input value={renameValue} onChange={(event) => setRenameValue(event.target.value)} /></label><button type="submit">Save name</button></form>}
          ...
          {reviewing === branch.id && <div className={styles.branchReview}>
            ...
            <button type="button" disabled={!reviewers.trim()} onClick={() => board.id && void requestBranchReview(board.id, branch.id, reviewers.split(/[\s,]+/).filter(Boolean), reviewNote).then(() => { setMessage("Review requested."); return load(); }).catch((caught) => setError(errorMessage(caught, "Review request failed.")))}>Request review</button>
            <div className={styles.buttonGrid}><button type="button" onClick={() => board.id && void reviewDesignBranch(board.id, branch.id, "approved", reviewNote).then(() => { setReviewing(null); setMessage("Branch approved."); return load(); }).catch((caught) => setError(errorMessage(caught, "Review failed.")))}>Approve</button><button type="button" onClick={() => board.id && void reviewDesignBranch(board.id, branch.id, "changes-requested", reviewNote).then(() => { setReviewing(null); setMessage("Changes requested."); return load(); }).catch((caught) => setError(errorMessage(caught, "Review failed.")))}>Request changes</button></div>
          </div>}
      ...
      {inactiveBranches.length > 0 && <section className={styles.inspectorSection}><h2>History</h2><div className={styles.branchList}>{inactiveBranches.map((branch) => <div className={styles.branchRow} key={branch.id}><div><GitBranch aria-hidden="true" /><span><strong>{branch.name}</strong><small>{branch.status}{branch.merge_description ? ` · ${branch.merge_description}` : ""}</small></span></div>{branch.status === "archived" && <button type="button" onClick={() => board.id && void restoreDesignBranch(board.id, branch.id).then(load).catch((caught) => setError(errorMessage(caught, "Restore failed.")))}>Restore</button>}</div>)}</div></section>}
