...
import type { BuilderRun, Effort } from './protocol';

export interface BuilderSession { runId: string; lease: string }
export const builderStatus = () => authenticatedFetch<{ enabled: boolean; remainingMicros: number; operator?: boolean }>('/api/builder');
...
export const createBuilderRun = (session: BuilderSession, prompt: string, effort: Effort, scope: string, boardId: string | null, roomId: string | null, selectionIds: string[]) =>
  builderRequest(session, 'create', { prompt, effort, scope, boardId, roomId, selectionIds });

export async function builderStep(session: BuilderSession, context: string, signal: AbortSignal, onProgress: (message: string) => void): Promise<BuilderRun> {
  ...
  try {
    while (true) {
      ...
      while ((newline = buffer.indexOf('\n')) >= 0) {
        ...
        const event = JSON.parse(line.slice(6)) as { type: string; message: string; run: BuilderRun };
