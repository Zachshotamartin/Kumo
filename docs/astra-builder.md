...
The signed-in **Kumo AI** button lives in the board header and opens a chat dock in the inspector column. It is absent from the boards dashboard. The editor and API modules load on demand. At narrower widths, secondary workspace controls move into a labeled dropdown. The Liveblocks free attribution remains visible; inspector controls have 80px plus safe-area scroll clearance. The builder has separate clearance above the badge.

## Use

Chat naturally, ask follow-up questions, and refine the same design. Visible response text streams into orange reply bubbles with paragraphs, headings, lists, bold text, and code formatting. Action receipts stay in an expandable list. Close/reopen retains the conversation in the current tab; New chat clears it after ending any unfinished request. Choose Selection, Current board, or Workspace; describe the result; choose low, medium, high, xhigh, or max effort. The model is always `gpt-6-astra`, default effort low. Changing effort during a run affects the next provider request. Real document changes, a labeled cursor, an optional Follow control, and the action log show progress. Manual canvas interaction disables Follow. Stop cancels inference and prevents new operations; a completed in-flight effect is still acknowledged.
...
Apply all migrations through `supabase/migrations/202609140002_builder_owner_allowance.sql` before deploying this release. All four builder tables and transition/pruning functions are restricted to the service role. Browser Supabase access remains revoked.
...
Do not send credentials in chat, logs, or the PR. Redeploy after changing runtime configuration. Disabling hosted inference blocks new create/step calls; already-running requests may finish or be stopped by their executor.

The owner has authorized live testing with the configured OpenAI key. CI uses deterministic inference replies and never makes paid inference requests.

## Spending, recovery, and operations

The verified Firebase identity `zachsm@alumni.stanford.edu` is exempt from monetary allowance checks. The API derives the exemption from the verified token, never from request fields. Its usage is recorded in separate `unmetered:` buckets, so it does not consume the free-user pool. Authentication, scope, operation checks, concurrency, and step limits still apply.

For other accounts, each request reserves allowance atomically against account, run, day, and month buckets. Limits include outstanding reservations. The server allows one active run per account/board, two concurrent sponsored provider requests, six model requests per run, up to 16,000 output tokens per request, and 100 created objects per run. Identity/IP rate limits precede inference. Inputs are bounded well below the long-context tier. Conservative byte-based reservations can stop a complex run before its nominal allowance is spent.
